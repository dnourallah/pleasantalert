...
export function _main (userParams) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (innerParams.input) {
      dom.addClass(document.body, pandClasses['has-input'])
    }
    ...
    for (let i = 0; i < inputTypes.length; i++) {
      const inputClass = pandClasses[inputTypes[i]]
    ...
    switch (innerParams.input) {
      ...
      case 'tel':
      case 'url':
        input = dom.getChildByClass(domCache.content, pandClasses.input)
        ...
        break
      case 'file':
        input = dom.getChildByClass(domCache.content, pandClasses.file)
        input.placeholder = innerParams.inputPlaceholder
        input.type = innerParams.input
        dom.show(input)
        break
      case 'range':
        const range = dom.getChildByClass(domCache.content, pandClasses.range)
        ...
        break
      case 'select':
        const select = dom.getChildByClass(domCache.content, pandClasses.select)
        select.innerHTML = ''
        ...
        break
      case 'radio':
        const radio = dom.getChildByClass(domCache.content, pandClasses.radio)
        radio.innerHTML = ''
        populateInputOptions = (inputOptions) => {
          inputOptions.forEach(([radioValue, radioLabel]) => {
            ...
            radioInput.name = pandClasses.radio
        ...
        break
      case 'checkbox':
        const checkbox = dom.getChildByClass(domCache.content, pandClasses.checkbox)
        const checkboxInput = this.getInput('checkbox')
        checkboxInput.type = 'checkbox'
        checkboxInput.value = 1
        checkboxInput.id = pandClasses.checkbox
        ...
        break
      case 'textarea':
        const textarea = dom.getChildByClass(domCache.content, pandClasses.textarea)
